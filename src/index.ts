#!/usr/bin/env node
/**
 * Fireflies.ai MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Client sends their own Fireflies API key as Bearer token.
...
app.get('/', (_req, res) => {
  ...
  res.json({
    ...
    version: '1.0.0',
    description:
      'Access Fireflies.ai meeting transcripts, summaries, action items, and AI analysis through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass your Fireflies API key as the Bearer token. No credentials are stored on this server.',
      header: 'Authorization: Bearer <your-fireflies-api-key>',
      howToGetKey:
        'Generate an API key at https://app.fireflies.ai → Settings → Developer API',
    },
    configTemplate: {
      mcpServers: {
        fireflies: {
          ...
          headers: {
            Authorization: 'Bearer <your-fireflies-api-key>',
          },
        },
      },
    },
    links: {
      health: '/health',
      documentation: 'https://financemcps.agenticledger.ai/fireflies/',
    },
  });
});
...
<head>
  ...
  <title>Fireflies MCP Server — AgenticLedger</title>
...
<body>
  ...
  <div class="container">
    <h1>Fireflies MCP Server</h1>
    <p class="subtitle">Access Fireflies.ai meeting transcripts, summaries, action items, and AI analysis through MCP tools.</p>

    <div class="card">
      <h2>How to get your Fireflies API key</h2>
      <ol class="steps">
        <li>Log in to <a href="https://app.fireflies.ai" target="_blank" rel="noopener">Fireflies.ai</a>.</li>
        <li>Go to <strong>Settings</strong>.</li>
        <li>Navigate to <strong>Developer API</strong>.</li>
    ...
    <div class="card">
      ...
      <label for="apiKey">Fireflies API Key</label>
...
function createMCPServer(client: FirefliesClient): Server {
  const server = new Server(
    { name: 'fireflies-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
...
<head>
  ...
  <title>Authorize \u2014 Fireflies MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>Fireflies MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>Fireflies MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`Fireflies MCP HTTP Server running on port ${PORT}`);
  console.log(`  MCP endpoint:   http://localhost:${PORT}/mcp`);
  console.log(`  Health check:   http://localhost:${PORT}/health`);
  console.log(`  Tools:          ${tools.length}`);
  console.log(`  Transport:      Streamable HTTP`);
  console.log(`  Auth:           Bearer passthrough (client provides Fireflies API key)`);
});
